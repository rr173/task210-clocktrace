...
func TestBug06_UnconfirmedEventCannotBeSealed(t *testing.T) {
	db, err := store.Open(t.TempDir() + "/clocktrace.db")
	if err != nil { t.Fatal(err) }
	defer db.Close()
	app := service.New(db)
	snap, err := app.Topology.CreateSnapshot("seal-state")
	if err != nil { t.Fatal(err) }
	...
	if err != nil { t.Fatal(err) }
	if err := app.Verdict.Seal(res.Event.ID, "premature"); err == nil { t.Fatal("unconfirmed event was sealed") }
}
